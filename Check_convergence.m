...
dt_list = [5,10,20,50,60]; % solution on coarser grid
dt_r = 2; % reference solution
nrun = length(dt_list);
err = NaN(size(dt_list));

solu_r = load(['Results/T=50_365/solution_',num2str(dt_r),'.mat'],'P','t','Cs_t','Cs_end');
Cs_r = solu_r.Cs_end;
x_r = solu_r.P.a;

for irun = 1:nrun
    dt = dt_list(irun);
    solu = load(['Results/T=50_365/solution_',num2str(dt),'.mat'],'P','t','Cs_t','Cs_end');
    Cs = solu.Cs_end;
    x = solu.P.a;
