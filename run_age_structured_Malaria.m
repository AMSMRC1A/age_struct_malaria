...
%% time evolution
[SH, EH, DH, AH, SM, EM, IM, Cm, Cac, Ctot] = age_structured_Malaria();
PH = SH+EH+DH+AH;
...
axis([0 age_max 0 max(Ctot(:,end))*1.1]);
grid on

subplot(1,2,2)
plot(a,Cac(:,end)./PH_final,'-r');
hold on;
plot(a,Cm(:,end)./PH_final,'-b');
plot(a,Ctot(:,end)./PH_final,'-k');
xlabel('age (years)')
...
title('Immun dist.');
axis_years(gca,age_max);
axis([0 age_max 0 max(Ctot(:,end)./PH_final)*1.1]);
grid on
