...
%% For D part ----- double integral (alpha, a, s)    
    ...
    D_alpha_a_x = @(alpha,a,x) exp(-muH_int(alpha)).*exp(-P.rD.*(alpha-a)).*rho(a).*P.h.*exp(-P.h.*(a-x)).*theta0(x);
    amax = @(alpha) alpha;
    xmax = @(alpha,a) a;
...
%%  For A part --- double + triple integral on (alpha, a, x)...
    A1_alpha_a_x = @(alpha,a,x) exp(-muH_int(alpha)).*exp(-P.rA.*(alpha-a)).*...
        (1-rho(a)).*P.h.*exp(-P.h.*(a-x)).*theta0(x);
    A2_alpha_a_x_s = @(alpha,a,x,s) exp(-muH_int(alpha)).*exp(-P.rA.*(alpha-a)).*...
        P.rD.*(1-phi(a)).*exp(-P.rD.*(a-x)).*rho(x).*P.h.*exp(-P.h.*(x-s)).*theta0(s);
    amax = @(alpha) alpha;
    xmax = @(alpha,a) a;
    smax = @(alpha,a,x) x;
